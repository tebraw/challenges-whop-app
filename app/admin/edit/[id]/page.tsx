...
export default function EditChallengePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const c = (await res.json()) as ChallengeFromApi;
        ...
        setForm({
          ...
          startAt: toLocal(new Date(c.startAt)),
          endAt: toLocal(new Date(c.endAt)),
          proofType: c.proofType,
          cadence: rules.cadence === "END" ? "END_OF_CHALLENGE" : (rules.cadence as "DAILY" | "END_OF_CHALLENGE") ?? "DAILY",
          maxParticipants: rules.maxParticipants,
          rewards: Array.isArray(rules.rewards) && rules.rewards.length > 0 ? rules.rewards : [{ place: 1, title: "", desc: "" }],
          policy: rules.policy && rules.policy.length > 0 ? rules.policy : DEFAULT_POLICY_TEXT,
          imageUrl: c.imageUrl || rules.imageUrl || "", // Prefer top-level imageUrl, fallback to rules
          difficulty: rules.difficulty ?? "BEGINNER",
        });
  ...
  const handleSubmit = async () => {
    ...
    if (!parsed.success) return alert(parsed.error.errors[0].message);
    ...
    const res = await fetch(`/api/challenges/${id}`, {
      ...
      body: JSON.stringify({
        ...
        imageUrl: form.imageUrl, // Include imageUrl at top level
        rules: {
          cadence: form.cadence,
          maxParticipants: form.maxParticipants,
          rewards: form.rewards,
          policy: form.policy,
          imageUrl: form.imageUrl, // Also include in rules for backward compatibility
  ...
  if (!id) return <main className="p-6">No ID in URL.</main>;
          ...
          {step === 6 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <CopyField value={`http://localhost:3000/c/${id}`} />
              </div>
            </div>
          )}
