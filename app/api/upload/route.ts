...
/**
 * Generate thumbnail from base64 image (browser-compatible version)
 */
async function generateThumbnailSimple(dataUrl: string): Promise<string> {
  try {
    // For now, return original - thumbnail will be generated by migration script
    // This keeps the upload fast and doesn't require canvas dependency
    return dataUrl;
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    // Generate thumbnail (lightweight version for list views)
    const thumbnail = await generateThumbnailSimple(dataUrl);
    
    return NextResponse.json({ 
      url: dataUrl,
      thumbnail: thumbnail, // Include thumbnail for future use
